...
class deep_analysis_module(dspy.Module):
    ...
    async def execute_deep_analysis_streaming(self, goal, dataset_info, session_datasets=None):
        ...
        try:
            ...
            # Helper: strip markdown fences and try multiple parse strategies
            def _parse_plan(raw: str) -> dict:
                """Try json.loads first, then ast.literal_eval. Strip markdown fences."""
                cleaned = raw.strip()
                # Remove ```json or ``` fences the LLM sometimes wraps around output
                ...
                # Try json.loads first (handles -> as invalid JSON → raises error properly)
                try:
                ...
                try:
                    ...
                    pass
                raise ValueError(f"Could not parse plan as dict. Raw: {raw[:200]}")

            try:
                plan_instructions = _parse_plan(deep_plan.plan_instructions)
                keys = [key for key in plan_instructions.keys()]
                
                # Strip @ prefixes from keys if the LLM added them
                plan_instructions = {k.lstrip("@"): v for k, v in plan_instructions.items()}
                keys = list(plan_instructions.keys())

                if not all(key in self.agents for key in keys):
                    raise ValueError(f"Invalid agent key(s) in plan instructions. Expected one of: {list(self.agents.keys())}. Got: {keys}")
                logger.log_message(f"Plan instructions parsed. Keys: {keys}", logging.INFO)
            except (ValueError, SyntaxError, json.JSONDecodeError) as e:
                logger.log_message(f"First parse attempt failed: {e}. Trying AI plan fixer...", logging.WARNING)
                try:
                    deep_plan = await self.deep_plan_fixer(plan_instructions=deep_plan.plan_instructions)
                    plan_instructions = _parse_plan(deep_plan.fixed_plan)
                    plan_instructions = {k.lstrip("@"): v for k, v in plan_instructions.items()}
                    keys = list(plan_instructions.keys())
                except (ValueError, SyntaxError, json.JSONDecodeError) as e2:
                    logger.log_message(f"Plan fixer also failed: {e2}", logging.ERROR)
                    raise e2
            ...
            # Check if we have valid API key (prefer OpenAI, fall back to Anthropic)
            ...
            try:
                # Use OpenAI by default (gpt-4.1 is capable for code synthesis)
                if openai_key:
                    thread_lm = dspy.LM("openai/gpt-4.1", api_key=openai_key, max_tokens=8000, temperature=0.5)
                else:
            ...
            logger.log_message(f"Error in deep analysis: {str(e)}\n{error_traceback}", logging.ERROR)
            print(f"CRITICAL STREAM ERROR: {error_traceback}")
            yield {
                "step": "error",
                "status": "failed",
