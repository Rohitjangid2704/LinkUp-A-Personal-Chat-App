        ...
        if (not current_analyzer or 
            ...
            try:
                ...
                if not enabled_agents_dict:
                    # Fallback: load actual Signature objects from the DB for the default planner agents.
                    # We CANNOT use plain strings here — dspy.Predict() requires a Signature class.
                    from src.agents.agents import load_user_enabled_templates_for_planner_from_db
                    fallback_session = session_factory()
                    try:
                        # Pass user_id=None; the function handles fallback to default planner agents
                        enabled_agents_dict = load_user_enabled_templates_for_planner_from_db(None, fallback_session)
                    finally:
